...
pdf("SUMMARY_Trends_vs_TimeSeriesLength.pdf", width=11, height=8.5)
par(mfrow=c(2,3), oma=c(2,4,2,0.25), mar=c(3,3,3,1))
for(posNeg in c(1,-1)){
  for(i in 1:length(VarsToPlot)){
    ...
    percentAllCutoffs_sig = (sum((posNeg*all_coef[indsToPlot_all, "b1"]) > 0 & all_coef[indsToPlot_all, "b1.p"] < 0.05, na.rm=TRUE) / N_total)*100
    ...
    for(j in 1:(length(yearGroupCutoffs)-1)){
      ...
      curPercent_sig = (sum((posNeg*X[, "b1"]) > 0 & X[, "b1.p"] < 0.05, na.rm=TRUE) / nrow(X))*100
    ...
    # make plot
    colSig = ifelse(posNeg==1, "palegreen", "skyblue")
    colAll = ifelse(posNeg==1, "palegreen4", "skyblue4")
    plot(c(1:5),allPercent_all, pch=19, cex=2, ylim=c(0,100), col=colAll, xaxt="n", ylab="", xlab="", xlim=c(0.5,5.5))
    abline(h=percentAllCutoffs_all, lwd=2, lty=2, col=colAll)
    axis(side=1, at=c(1,2,3,4,5), labels=allLabels)
    ...
    if(i == 1){
      ...
      mtext(side=2, Ylab, line=2.5, cex=1.25)
      legend(x="topleft", legend=c("All", "Significant"), text.col=c(colAll, colSig), bty="n", cex=1.25)
    }
  }
  mtext(side=2, "% of Lakes", outer=TRUE, line=2, cex=1.5)
}
dev.off()